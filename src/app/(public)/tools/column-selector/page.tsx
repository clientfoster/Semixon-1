...
export default function ColumnSelectorPage() {
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setFiles((prev) => {
            ...
            // Initialize selected columns for new files (select all by default)
            const newSelectedColumns = { ...selectedColumns };
            newFiles.forEach((file, index) => {
                const globalIndex = prev.length + index;
                newSelectedColumns[globalIndex] = file.headers;
            });
        ...
        return new Promise((resolve) => {
            ...
            reader.onload = (e) => {
                ...
                if (file.name.endsWith('.csv')) {
                    // For CSV, we can use XLSX.read with type 'string' if readAsText was used, 
                    // but readAsArrayBuffer is safer for all formats with XLSX library
    ...
    const toggleColumn = (fileIndex: number, column: string) => {
        setSelectedColumns((prev) => {
            const currentSelected = prev[fileIndex] || [];
            const isSelected = currentSelected.includes(column);

            let newSelected;
            if (isSelected) {
                newSelected = currentSelected.filter((c) => c !== column);
            } else {
                newSelected = [...currentSelected, column];
            }

            return { ...prev, [fileIndex]: newSelected };
        });
    };

    const toggleAllColumns = (fileIndex: number, checked: boolean) => {
        setSelectedColumns((prev) => ({
            ...prev,
            [fileIndex]: checked ? files[fileIndex].headers : [],
        }));
    };

    const handleProcess = () => {
        // Check if any columns are selected
        const hasSelection = Object.values(selectedColumns).some((cols) => cols.length > 0);
        if (!hasSelection) {
            alert('Please select at least one column from any file!');
            return;
        }
        setProcessed(true);
    };
    ...
    const handleDownload = async () => {
        if (files.length === 1) {
            ...
            const fileName = file.name.replace(/\.[^/.]+$/, "") + "_processed" + (extension === '.csv' ? '.csv' : '.xlsx');
            XLSX.writeFile(workbook, fileName);
        } else {
            const zip = new JSZip();

            files.forEach((file, index) => {
                const columns = selectedColumns[index] || [];
        ...
        <div className="min-h-screen bg-slate-50 bg-royal-pattern-light py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto space-y-8">
                <div className="text-center space-y-4">
                    <h1 className="text-4xl font-bold text-gradient-royal pb-1">Column Selector</h1>
                    <p className="text-lg text-slate-600 max-w-2xl mx-auto">
                        Upload CSV or XLSX files to merge columns and download processed files.
                    </p>
                </div>
